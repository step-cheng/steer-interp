...
DictionaryStash = namedtuple("DictionaryStash", ["embed", "attns", "mlps", "resids", "final_ln"])
    ...
    for i, layer in tqdm(
        ...
        resids.append(
            resid := Submodule(
                name=f"resid_{i}",
                submodule=layer,
                is_tuple=True, # could this actualy be False? it was true before
            )
        )
...
def _load_generic_submodules(
    ...
    for i, layer in tqdm(
        ...
        attns.append(
            attn := Submodule(
                name=f"attn_{i}", submodule=layer.self_attn
            )
        )
        mlps.append(
            mlp := Submodule(
                name=f"mlp_{i}", submodule=layer.mlp,
            )
        )
        resids.append(
    ...
    possible_names = ["norm", "final_layer_norm"]
    for name in possible_names:
        if hasattr(model.model, name):
            layernorm_module = getattr(model.model, name)
            break
    else:
        raise ValueError("norm layer not found")
    final_ln = Submodule(name="final_ln", submodule=layernorm_module)

    if separate_by_type:
        return DictionaryStash(embed, attns, mlps, resids, final_ln)
    else:
        submodules = ([embed] if include_embed else []) + [
            x
            for layer_dictionaries in zip(attns, mlps, resids)
...
def load_submodules(
    ...
    if model_name == "EleutherAI/pythia-70m-deduped":
        ...
        return _load_generic_submodules(
            model,
            thru_layer=thru_layer,
            separate_by_type=separate_by_type,
            include_embed=include_embed,
