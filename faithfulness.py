...
def forward_with_circuit(model, submodules: list[Submodule], steer_submod: Submodule, steer_vec, inputs, input_masks, node_names, base_acts):
    ...
    with t.no_grad(), model.trace(inputs):
        ...
        act_deltas = t.zeros(
            ...
            device=model.device, dtype=t.float32
        )
        for submodule in submodules:
            ...
            if submodule.is_resid:
                ...
                q_mask = (1-input_masks[f"{submodule.name}_q"]['mask']).to(submodule.device)    # get edges that are not in circuit
                num_upstream = input_masks[f"{submodule.name}_q"]['slice']
                q_deltas = einsum(act_deltas[:,:,:num_upstream], q_mask[:num_upstream].float(),         # apply deltas to edges not in circuit
                                "b s n_upstream d, n_upstream -> b s d")

                k_mask = (1-input_masks[f"{submodule.name}_k"]['mask']).to(submodule.device)
                k_deltas = einsum(act_deltas[:,:,:num_upstream], k_mask[:num_upstream].float(), 
                                "b s n_upstream d, n_upstream -> b s d")

                v_mask = (1-input_masks[f"{submodule.name}_v"]['mask']).to(submodule.device)
                v_deltas = einsum(act_deltas[:,:,:num_upstream], v_mask[:num_upstream].float(), 
                                "b s n_upstream d, n_upstream -> b s d")
                
                # print((pre_ln_act + q_deltas)[0,-1], base_ln_ins[submodule.name][0,-1])
                
                # if not t.allclose((pre_ln_act + q_deltas)[0,-1], base_ln_ins[submodule.name][0,-1]):
                #     mask = t.argwhere(~t.isclose((pre_ln_act + q_deltas)[0,-1], base_ln_ins[submodule.name][0,-1]))
                #     print(len(mask))

                # assert t.allclose((pre_ln_act + q_deltas)[0,-1], base_ln_ins[submodule.name][0,-1], atol=1e-5), submodule.name
                # assert t.allclose((pre_ln_act + k_deltas)[0,-1], base_ln_ins[submodule.name][0,-1], atol=1e-5), submodule.name
                # assert t.allclose((pre_ln_act + v_deltas)[0,-1], base_ln_ins[submodule.name][0,-1], atol=1e-5), submodule.name
                new_q_in = rms_norm_fn(ln_submod, pre_ln_act.float() + q_deltas)
                new_k_in = rms_norm_fn(ln_submod, pre_ln_act.float() + k_deltas)
                new_v_in = rms_norm_fn(ln_submod, pre_ln_act.float() + v_deltas)
                submodule.set_qkv_inputs_separate(new_q_in.to(t.bfloat16), new_k_in.to(t.bfloat16), new_v_in.to(t.bfloat16))

                head_acts = submodule.get_per_head_outputs().clone()
                head_acts = submodule.handle_post_attn_mlp_norm(model, head_acts)
                for h in range(num_heads):
                    # assert t.allclose(base_acts[f'{submodule.name}_h{h}'], head_acts[:,:,h,:], atol=1e-5), f"{submodule.name}_h{h}"
                    key = f"{submodule.name}_h{h}"
                    idx = node_names.index(key)
                    # print(f'setting idx {idx} with delta {(base_acts[key].to(submodule.device) - head_acts[:,:,h,:])[0,-1]}')
                    # delta = steered - modified (will be ~0 if input was fully corrupted)
                ...
                deltas = einsum(act_deltas[:,:,:num_upstream], mask[:num_upstream].float(), 
                                "b s n_upstream d, n_upstream -> b s d")
                # print((pre_ln_act + deltas)[0,-1], base_ln_ins[submodule.name][0,-1])
                new_act = rms_norm_fn(ln_submod, pre_ln_act.float() + deltas)
                submodule.set_in_activation(new_act.to(t.bfloat16))
...
def get_input_masks(edges, submodules: list, steer_submod: Submodule, num_attn_heads: int):
    """
    ...
    for submodule in submodules:
        if submodule.is_attn:
            num_heads = submodule.config.num_attention_heads
            for h in range(num_heads):
    ...
    for submodule in submodules:
        if submodule.is_attn:
            ...
            for l in 'qkv':
                input_masks[f"{submodule.name}_{l}"] = {
                    'mask': t.zeros(len(node_names)-1, dtype=t.bfloat16),
                    'slice': 1 + (layer_idx-steer_layer_idx)*(num_attn_heads+1)
                }
                input_masks[f"{submodule.name}_{l}"]['mask'] = t.zeros(len(node_names)-1, dtype=t.bfloat16)
...
if __name__ == '__main__':
    ...
    input_masks, node_names = get_input_masks(edges, steer_submodules, resid_submod, model.config.num_attention_heads)
    if args.invert:
